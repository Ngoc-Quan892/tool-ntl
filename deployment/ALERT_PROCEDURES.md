...
2. Cấu hình:
   ```bash
   export SLACK_WEBHOOK_URL="https://REDACTED_SLACK_HOOKS_DOMAIN/services/YOUR/WEBHOOK/URL"
   ```
...
3. Network connectivity?
   ```bash
   docker-compose exec backend curl -I https://REDACTED_SLACK_HOOKS_DOMAIN
...
1. Webhook URL đúng format?
   ```
   https://REDACTED_SLACK_HOOKS_DOMAIN/services/YOUR/WEBHOOK/URL
   ```
