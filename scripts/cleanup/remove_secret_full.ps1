...
Write-Host "Checking mirror for remaining 'hooks.slack.com' occurrences..." -ForegroundColor Cyan
try {
  $found = & git grep -n "hooks.slack.com" --all 2>$null
...
if ($found) {
  ...
  Write-Host "No occurrences of 'hooks.slack.com' found in mirror." -ForegroundColor Green
}
